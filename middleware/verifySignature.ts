...
export const verifyHmacSignature = async (
  ...
  else if (Object.keys(request.params).length !== 0) requestBody = request.params;
  else if (Object.keys(request.body).length === 0 && Object.keys(request.params).length === 0) requestBody = request.method;
