...
describe("Insert /friend/", () => {
  it("should create a friend record", async () => {
    ...
    const hmacSignature = generateHmacSignature(JSON.stringify(friendBody), process.env.SECRET_KEY);
    const res = await request(app).post("/friend").send(friendBody).set('Friends-Life-Signature', hmacSignature);
...
describe("GET /friend/", () => {
  it("should return all friends", async () => {
    ...
    const res = await request(app).get("/friend").set('Friends-Life-Signature', hmacSignature);
    expect(res.statusCode).toBe(200);
...
describe("DELETE /friend/:friendId", () => {
  it("should delete a friend", async () => {
    if(process.env.SECRET_KEY === undefined) return false;
    const hmacSignature = generateHmacSignature(JSON.stringify({ friendId: friendIds[0] }), process.env.SECRET_KEY);
    const res = await request(app).delete(`/friend/${friendIds[0]}`).set('Friends-Life-Signature', hmacSignature);
    expect(res.statusCode).toBe(204);
  });
});
